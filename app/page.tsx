...
const fetchProduct = async (genericstring: string): Promise<ApiResponse> => {
  try {
    ...
    const { data } = await axios.post<ApiResponse>(`${API_BASE_URL}/Productos/productReadBarCode`, requestData, {
      headers: {
...
function BuscadorProductos() {
  ...
  const calculatePrice = (product: ProductResponse) => {
    const basePrice = Number.parseFloat(product.PrecioClaseDefault.toFixed(3))
  ...
  return (
    <div
      className={`min-h-screen bg-gray-100 ${isFullScreen ? "fixed inset-0 z-50" : ""}`}
      ...
      {isClient ? (
        <div className="container mx-auto p-2 sm:p-3 md:p-4">
          <main>
            ...
            {isError && (
              <div className="text-center text-lg sm:text-xl md:text-2xl text-red-600">Error al buscar el producto</div>
            )}
            {productNotFound && (
              <div
                className="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-2 sm:p-3 mb-2 sm:mb-3"
                role="alert"
              >
                    ...
                    <div>
                      ...
                      <div className="bg-white-100 p-2 sm:p-3 md:p-4 rounded-lg">
                        <ul className="space-y-1 sm:space-y-2 text-base sm:text-lg md:text-xl lg:text-2xl text-gray-600">
                          ...
                          <li>
                            <span className="font-semibold">Empaque:</span> {product.Empaque}
                          </li>
                          <li>
                          ...
                          <li>
                            ...
                            {product.AIVA ? `${product.PIVA}%` : "No aplica"}
                          </li>
                        </ul>
                      </div>
                    </div>
